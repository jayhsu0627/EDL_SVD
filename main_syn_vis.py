...
def validate_and_convert_image(image, target_size=(128, 128)):
    if image is None:
...
def make_train_dataset(args):
    ...
    dataset = SyncDataset(args.video_folder + '/train/' + str(args.width), max_frames=args.num_frames)

    return dataset

def make_test_dataset(args):
    # dataset = MultiIlluminationDataset("/sdb5/data/test/",
    #                                     frame_size=25, sample_n_frames=16)
    # dataset = SyncDataset(args.video_folder + '/test', max_frames=16)
    dataset = SyncDataset(args.video_folder + '/test/' + str(args.width), max_frames=args.num_frames)
...
def main():
    ...
    print(f"Pipeline dtype: {pipeline.dtype}")
    ...
    for batch_idx, batch in enumerate(test_dataloader):
        ...
        video_frames = pipeline(
            ...
            decode_chunk_size=8, #8
            motion_bucket_id=127, # 127
            fps=7,
        ...
        out_file = os.path.join(
            val_save_dir,
            f"test_inf_"+ str(batch_idx) + ".mp4",
        )
        out_file_gt = os.path.join(
            val_save_dir,
            f"test_gt_"+ str(batch_idx) + ".mp4",
        )
        ...
        # export_frames_to_pngs(video_frames, out_file)
        ...
        # export_frames_to_pngs(video_frames, out_file_gt)
        # break
        # concatenate image
