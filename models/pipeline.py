...
class StableVideoDiffusionMaskPipeline(StableVideoDiffusionPipeline):
    ...
    def __call__(
        ...
        # print('G-buffer:', g_buffer[0].shape)
        # print('G-buffer:', g_buffer[1].shape)
        # print('G-buffer:', g_buffer[2].shape)
        # print('G-buffer:', g_buffer[3].shape)
        # print('G-buffer:', g_buffer[4].shape)
        ...
        depths_exp = depths.repeat(1, 3, 1, 1)  # Expand along dim=2 to 3 channels

        # scribbles_exp = scribbles.repeat(1, 1, 3, 1, 1)  # Expand along dim=2 to 3 channels
        scribbles_exp = scribbles.repeat(1, 3, 1, 1)  # Expand along dim=2 to 3 channels
        ...
        # print('encode_vae_video:', rgbs.shape)
        # print('encode_vae_video:', depths_exp.shape)
        # print('encode_vae_video:', normals.shape)
        # print('encode_vae_video:', albedos.shape)
        # print('encode_vae_video:', scribbles_exp.shape)

        with torch.no_grad():
            enc_rgb = self.encode_vae_video(rgbs.to(self.vae.dtype), chunk_size=decode_chunk_size).unsqueeze(0)  # [1, t, c, h, w]
            enc_depth = self.encode_vae_video(depths_exp.to(self.vae.dtype), chunk_size=decode_chunk_size).unsqueeze(0)  # [1, t, c, h, w]
            enc_nrm = self.encode_vae_video(normals.to(self.vae.dtype), chunk_size=decode_chunk_size).unsqueeze(0)  # [1, t, c, h, w]
            enc_alb = self.encode_vae_video(albedos.to(self.vae.dtype), chunk_size=decode_chunk_size).unsqueeze(0)  # [1, t, c, h, w]
            enc_scb = self.encode_vae_video(scribbles_exp.to(self.vae.dtype), chunk_size=decode_chunk_size).unsqueeze(0)  # [1, t, c, h, w]
        ...
        while idx_start < num_frames - overlap:
            ...
            add_latents = add_latents[:, idx_start:idx_end]
            ...
            with self.progress_bar(total=num_inference_steps) as progress_bar:
                for i, t in enumerate(timesteps):
                    ...
                    print(latent_model_input.shape, video_latents_current.shape, add_latents.shape)


                    # latent_model_input = torch.cat(
                    #     [latent_model_input, video_latents_current], dim=2
                    # )
                    latent_model_input = torch.cat([latent_model_input, video_latents_current, add_latents], dim=2)
